...
export class OnlineNonBengaliBillArrearDetailsService {
  ...
  private getNonBengaliDetailsReport(data: any, reportObj: any,){
    ...
    let uniqueZone=[... new Set(data.nonbengliDetailsDataList.map(item=>item.zoneName))];
    console.log("Unique Zone",uniqueZone);
    ...
    uniqueZone.forEach(item=>{
      if(item){ 
        ...
        let serial = 0;
        let zoneList=data.nonbengliDetailsDataList.filter(p=>p.zoneName==item);
        let uniqueLocation=[...new Set(zoneList.map(item=>item.locationCode || item.zoneName))];
        uniqueLocation.forEach((location)=>{
          let nonBengaliArrearGroupByZone=data.nonbengliDetailsDataList.filter(x=>x.locationCode==location);
          ...
          if(nonBengaliArrearGroupByZoneLength>0){
            ...
            nonBengaliArrearGroupByZone.forEach(value=>{
              ...
              nonBengaliTotal += value.totalAmount;   
              ...
              phase.table.body.push(
                [
                  ...
                  {
                    text: `${this.translateNumber(value.totalAmount.toFixed(2))}`,
                    border: [true, true, true, true],
                    style: ['setRight', ],
                    colSpan: 1,
                  },
