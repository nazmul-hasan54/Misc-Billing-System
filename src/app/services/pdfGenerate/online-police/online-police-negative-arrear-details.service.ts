...
export class OnlinePoliceNegativeArrearDetailsService {
  defaultColor = '#111';
  constructor(
  ) { }
 ...
 private getDocumentDefinition(data: any, reportObj: any) {
  return {
    info: { 
      title: 'Online Police Negative Arrear Details',
      author: 'BPDB',
      subject: 'Online Police Negative Arrear Details',
...
private getOnlineMinistryArrearDetailsHeading(data:any, reportObj){
  ...
  let negativeData = data.filter(x => (((x.arrearLps+x.arrearPrincipal+x.arrearVat)-x.totalReceiptArrear)+(x.currPrincipal+x.currLps+x.currVat)-
  (x.currReceiptPrincipal + x.currReceiptVat)) <= -1 );
  //let totalCustomer = data.length;
  let totalCustomer = negativeData.length; 
...
private getOnlineMinistryArrearDetailsReport(data: any, reportObj: any) {
  ...
  let ministryDataByZone = data.filter(p => p.zoneCode != ' ' && ((((p.arrearPrincipal+p.arrearLps+p.arrearVat)-p.totalReceiptArrear)+(p.currPrincipal+p.currLps+p.currVat)-
  (p.currReceiptPrincipal+p.currReceiptVat)) <= -1))
