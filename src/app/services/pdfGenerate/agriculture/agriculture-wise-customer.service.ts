...
export class AgricultureWiseCustomerService {
  defaultColor= "#0c0d0d"
  ...
  private getHeading(billMonth: any) {
    ...
    //           colSpan: 2,
    //           bold: true,
    //           style: [setAgricultureAndPoultryStyles.setBanglaSubHeading],
    //           margin: [0, -5, 0, 0],
    //         },
    //         {},
    //         {
  ...
  private getAgriCultureCustomerInfo(data:any) {
    ...
    uniqueKrishiLocations.forEach((location) => {
      let customerGroupByLocation = KrishiCustomers.filter(x => x.locationCode
        ==location);
