...
export class OnlineMinistryNegativeArrearCrvService {
  defaultColor = '#111';
  ...
  constructor() { }
    ...
    private getDocumentDefinition(data: any, reportObj: any) {
      return {
        info: {
          title: 'Online Ministry Nrgative Arrear With CRV',
          author: 'BPDB',
          subject: 'Online Ministry Nrgative Arrear With CRV',
        ...
        header: this.getOnlineMinistryNegativeCRVArrearHeading(data, reportObj),
        content: [
          this.getOnlineMinistryNegativeCRVArrearReport(data, reportObj)
        ],
    ...
    private getOnlineMinistryNegativeCRVArrearHeading(data:any, reportObj:any){
      let ministryDataByZone = data.filter(p => p.zoneCode != ' ' && ((((p.arrearPrincipal+p.arrearLps+p.arrearVat)-p.totalReceiptArrear)+(p.currPrincipal+p.currLps+p.currVat)-(p.currReceiptPrincipal+p.currReceiptVat)) <= -1));
      let totalCustomer = ministryDataByZone.length; 
      // let ministryName=data[0].ministryName;
      let ministryName;
    
      if(reportObj.ministry=='0'){
        ministryName='All Ministry';
       }
       else{
        ministryName=data[0].ministryName;
       }
          ...
          table: {
            ...
            body: [
              ...
              [
                ...
                {},
                {
                  text: `Ministry Negative Arrear With CRV`,
  ...
  private getOnlineMinistryNegativeCRVArrearReport(data: any, reportObj: any) {
    ...
    const phases = {
      table: {
        dontBreakRows: true,
        headerRows: 1,
        widths: [25,'*',40, 60,'auto', 75, 65,65,80],
        body: [
          [
            {
              text: 'SL No',
            ...
            {
              text: `Total Collection(${month}-${year})`,
              style: ['setBlack','setBold'],
              border: [true, true, true, true],
            },
            {
    ...
    let cityCorpoName="";
    let zoneNameValue = "";
    let prevRcptArrValue = 0;
    let currMonthBillValue = 0;
    let totalRcptValue = 0;
    let currRcptArrValue = 0;
    ...
    //{
      let uniqueZoneCode=[...new Set(ministryDataByZone.map(item=>item.zoneName))];
      

      uniqueZoneCode.forEach((zoneData)=>{
        let middleInsertProviderNo = 10;
        let customerGroupByZone=ministryDataByZone.filter(x=>x.zoneName==zoneData)
        ...
        let zoneTotalcurrentReceiptArrear=0;
          ...
          customerGroupByZone.forEach(element => {
            ...
            if(middleInsertProviderNo < customerGroupByZoneLength){
              middleInsertProviderNo = providerNo%32 ==0 ? (middleInsertProviderNo +32) : (middleInsertProviderNo + 0) 
            }

            previousReceiptArrear=(arrearPrincipal+arrearLps+arrearVat)-totalReceiptArrear;
            ...
            //let formatCurrentReceiptArrear = (currentReceiptArrear === -0) ? 0 : currentReceiptArrear;

            
            //if(currentReceiptArrear <0){
            ...
            // if(previousReceiptArrear < 0  || currentMonthBill < 0 || totalReceipt < 0 || currentReceiptArrear < 0 ){
            //if(currentReceiptArrear != -0){
              serial++;
