...
import { ministryDetailsDefaultStyle, ministryDetailsHeaderMargin, ministryDetailsImageMargin, ministryDetailsPageMargin, ministryDetailsStyle, misAllMinistryCenterwiseSummaryDefaultStyle, misAllMinistryCenterwiseSummaryPageMargin, misMinistryArrearStyle, misMinistrySummaryHeaderMargin, misMinistrySummaryStyle, setAgricultureAndPoultryStyles, setBillStyles, setHeading, setPdfMakeFonts, setPouroshovaAndCityStyle, setSubHeading, setSubSetHeading, } from '../config/pdfMakeConfig';
(<any>pdfMake).vfs = pdfFonts.pdfMake.vfs;

@Injectable({
  providedIn: 'root'
})
export class OnlineMinistryArrearService {
  defaultColor = '#111';
    ...
    data.forEach(ministryData => {
      ...
      let { ministryName,ministryNameBn,ministryCode, currVat, currLps, currReceiptVat,currReceiptPrincipal, currPrincipal,arrearPrincipal,arrearLps,arrearVat,
        ...
        if(((currPrincipal+currLps+currVat+arrearPrincipal+arrearLps+arrearVat)-(currReceiptPrincipal+currReceiptVat+arrearReceiptVat+arrearReceiptPrincipal))<0){
          ...
          if((zoneCode=='0' || zoneCode=='4') && reportObj.billMonth>='202311'){
      ...
      phases.table.body.push(
        [
          ...
          {
            text: `${this.translateNumber(Number(totalArrear/10000000).toFixed(2))}`,
            border: [true, true, true, true],
            style: [],
            alignment: 'left',
            margin: [0, 0, 0, 0],
    ...
    phases.table.body.push(
      [
        ...
        },
        {
          border: [true, true, true, true],
          style: [],
          text: `${this.translateNumber(Number(grandTotalArrear/10000000).toFixed(2))}`,
          margin: [0, 0, 0, 0],
        },
        {
  ...
  // getOnlineMinistryArrearReportWithoutString(data:any,reportObj:any){
  //   let billMonth = reportObj.billMonth;
  //   let year = (dayjs(billMonth).format("YYYY"));
  //   let month: string;
