...
@Component({
  selector: 'ngx-suplimentary-bill-sr',
  templateUrl: './suplimentary-bill-sr.component.html',
  styleUrls: ['./suplimentary-bill-sr.component.scss']
})
export class SuplimentaryBillSrComponent implements OnInit {
  ...
  constructor(
    ...
    private _suplimentaryBillsr: ReportBillService,
  ...
  onReport(billNumber, customerNumber) {
    this._penaltyService.getPenaltyBillSrPrint(billNumber, customerNumber).subscribe(res => {
      ...
      this.docData = this._suplimentaryBillsr.generatePdf(this.penaltyBillPrintData);
