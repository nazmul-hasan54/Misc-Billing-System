...
import { PrivateMinistryWiseReportService } from '../../../../services/pdfGenerate/ministry/private-ministry-wise-report.service';

@Component({
  selector: 'ngx-private-ministry-wise-customer-report',
  templateUrl: './private-ministry-wise-customer-report.component.html',
  styleUrls: ['./private-ministry-wise-customer-report.component.scss']
})
export class PrivateMinistryWiseCustomerReportComponent implements OnInit {
  ...
  constructor(
    ...
    private _pdfService: PrivateMinistryWiseReportService,
  ...
  public onReport() {
    ...
    let utilityObj = {
      ...
      this._service.getPrivateAndMinistryWiseCustReport(this.form.value,utilityObj.billMonth, this.formCon.zoneCode.value, this.form.value.zoneCode !="0" ? this.form.value.locationCode : "0", this.form.value.isRebate).subscribe(res => {
        this.ministryData = res;        ;
        console.log("data",this.ministryData)
        // if (this.ministryData.length > 0) {

          this.docData = this._pdfService.generatePdf(this.ministryData, utilityObj);
