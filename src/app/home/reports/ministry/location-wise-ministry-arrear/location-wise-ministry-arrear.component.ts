...
@Component({
  selector: 'ngx-location-wise-ministry-arrear',
  templateUrl: './location-wise-ministry-arrear.component.html',
  styleUrls: ['./location-wise-ministry-arrear.component.scss']
})
export class LocationWiseMinistryArrearComponent implements OnInit {
  ...
  onReport(){
    ...
    let reportObj = {
      // this._datePipe.transform(this.f.reportDate.value, 'yMM')
      //billMonth:this.f.reportDate.value,
      billMonth: this._datePipe.transform(this.f.billMonth.value, 'yMM'),
    }

    this._ministryService.getAllLocationWiseMinistryArrear(this.f.zoneCode.value, this.f.circleCode.value, this.f.locationCode.value, reportObj.billMonth).subscribe((res: any) => {
