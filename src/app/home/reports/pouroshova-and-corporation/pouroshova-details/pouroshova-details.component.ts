...
import { PouroshovaDetailsService } from '../../../../services/pdfGenerate/pouroshova-and-city-corporation/pouroshova-details.service';
import { dateFormatForReport } from '../../../../@core/utils/utiliy';

@Component({
  selector: 'ngx-pouroshova-details',
  templateUrl: './pouroshova-details.component.html',
  styleUrls: ['./pouroshova-details.component.scss']
})
export class PouroshovaDetailsComponent implements OnInit {
  ...
  constructor(
    ...
    private _pouroshovaDetails: PouroshovaDetailsService,
...
private getLocationDDByZone(zoneCode: string){
  ...
  onChangeExportType(event:any){
    if(event==1){
      let date=new Date();
      let fileName='Pouroshova Details '+dateFormatForReport(date);
      ...
      this._excelService.downloadExcelFile(excelObj, 'Pouroshova Details');
    }
  }
  ...
  //         this.docData=this._pouroshovaDetails.generatePdf(this.pouroshovaAndCityCorporDetails, reportObj,''); 
