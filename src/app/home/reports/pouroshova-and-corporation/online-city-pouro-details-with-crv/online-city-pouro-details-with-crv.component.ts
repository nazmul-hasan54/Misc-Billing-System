...
import { OnlineCityPouroCrvDetailsService } from '../../../../services/pdfGenerate/pouroshova-and-city-corporation/online-city-pouro-crv-details.service';
import { OnlineCityPouroCrvSummaryService } from '../../../../services/pdfGenerate/pouroshova-and-city-corporation/online-city-pouro-crv-summary.service';

@Component({
  selector: 'ngx-online-city-pouro-details-with-crv',
  templateUrl: './online-city-pouro-details-with-crv.component.html',
  styleUrls: ['./online-city-pouro-details-with-crv.component.scss']
})
export class OnlineCityPouroDetailsWithCrvComponent implements OnInit {
  ...
  constructor(
    ...
    private _pdfDetailsService: OnlineCityPouroCrvDetailsService,
    private _pdfSummaryService: OnlineCityPouroCrvSummaryService
  ) { }
  ...
  onChangeExportType(event: any) {
    if (event == 1) {
      ...
      //if(this.fCon.reportType.value == '1'){
        this.fileName ="Online Pouroshova and City Corporation CRV Details ";
      // }else {
      //   this.fileName ="Online Pouroshova and City Corporation Payment Details ";
      // }
      ...
      setTimeout(() => {
        //if(this.fCon.reportType.value == '1'){
          this.exporter =  this._excelService.downloadExcelFile( excelObj,"Online Pouroshova and City Corporation CRV Details");
        // }else{
  ...
  onReport() {
    ...
    if (this.form.invalid) {
      ...
      this._service.getOnlineCityAndPouroCRVDetails(utilityObj.billMonth,this.fCon.zoneCode.value,this.fCon.locationCode.value,this.fCon.reportType.value).subscribe((res:any) => {
      this.onlinePouroshovaAndCityData = res ;
            if(this.fCon.reportType.value == '1'){
              if (this.onlinePouroshovaAndCityData.cityCoporationDataList != null &&  this.onlinePouroshovaAndCityData.pouroshovaDataList != null) {
                this.docData = this._pdfSummaryService.generatePdf(this.onlinePouroshovaAndCityData,this.fCon.billMonth.value,this.zoneDataList,this.cityCorporationDataList);
              ...
              if (this.onlinePouroshovaAndCityData.cityPouroshovaDetailsDataList.length>0) {
                this.docData = this._pdfDetailsService.generatePdf(this.onlinePouroshovaAndCityData.cityPouroshovaDetailsDataList,this.fCon.billMonth.value);
                this.docData.getBase64((base64Data) => {
                  this.report = base64Data;
                  this.documentTitle = this.docData.docDefinition.info.title;
                  this.isTrue = true;
